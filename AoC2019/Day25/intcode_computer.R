                        ...
                        public = list(
                          ...
                          run = function(input=NULL, save=T, print=F) {
                            ...
                            while (self$status == "running") {
                              self$step(input, save=save, print=print)
                              input <- NULL
                            }
                          },
...
Droid <- R6::R6Class("Droid",
                     public = list(
                       ...
                       start = function() {
                         self$computer$run(print = T, save=T)
                         self$message = unlist(self$computer$outputs) %>% intToUtf8()
                         self$computer$outputs <- list()
                       },
                       
                       action = function(input, print=T) {
                         
                         input = utf8ToInt(paste0(input,"\n"))
                         self$computer$outputs<-list()
                         for (inp in input) {
                           self$computer$run(inp, print = print, save=T)
                         }
                         self$message = unlist(self$computer$outputs) %>% intToUtf8()
                       }
                     )
